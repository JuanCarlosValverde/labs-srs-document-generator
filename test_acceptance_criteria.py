...
def test_acceptance_criteria():
    ...
    rent_roll_files = [f for f in rent_roll_files if not f.name.endswith("_fields.csv")]
    ...
    comparables_files = [f for f in comparables_files if not f.name.endswith("_fields.csv")]
    ...
    expenses_files = [f for f in expenses_files if not f.name.endswith("_fields.csv")]
    ...
    if expenses_files:
        ...
        print(f"   ✅ Found {expense_count} expenses with {len(categories)} categories in {sample_file.name}")
    ...
    tenant_files = [f for f in tenant_files if not f.name.endswith("_fields.csv")]
    ...
    if tenant_files:
        ...
        demographic_fields = ['age_range', 'household_size', 'income_range', 'employment_status', 'credit_score_range']
        has_demographics = all(field in sample_tenant for field in demographic_fields)
        ...
        print(f"   ✅ Found {tenant_count} tenants with demographic data in {sample_file.name}")
    else:
    ...
    if noi_files:
        ...
        # Check date range
        if noi_data:
            first_date = datetime.fromisoformat(noi_data[0]['period_start']).date()
            last_date = datetime.fromisoformat(noi_data[-1]['period_start']).date()
            years_covered = (last_date - first_date).days / 365.25
        
        criteria_results["noi_data"] = {
            "count": noi_count,
            "years_covered": years_covered if noi_data else 0,
            "meets_criteria": 3 <= years_covered <= 5 and noi_count >= 36,  # At least 3 years of monthly data
            "file": sample_file.name
        }
        print(f"   ✅ Found {noi_count} NOI records covering {years_covered:.1f} years in {sample_file.name}")
    else:
    ...
    market_files = [f for f in market_files if not f.name.endswith("_fields.csv")]
    ...
    if market_files:
        ...
        has_statistics = all(field in sample_market for field in ['avg_sale_price', 'avg_cap_rate', 'avg_occupancy_rate'])
        ...
        print(f"   ✅ Found {market_count} market analyses with statistics in {sample_file.name}")
    else:
    ...
    if rent_roll_files:
        ...
        has_empty_cells = any(not unit.get('notes', '').strip() for unit in units)
        has_special_chars = any('!' in str(unit.get('unit_number', '')) for unit in units)
        has_large_numbers = any(float(unit.get('rent_amount', 0)) > 10000 for unit in units)
        
        criteria_results["edge_cases"] = {
            "has_empty_cells": has_empty_cells,
            "has_special_chars": has_special_chars,
            "has_large_numbers": has_large_numbers,
            "meets_criteria": has_empty_cells or has_special_chars or has_large_numbers,
            "file": sample_file.name
        }
        print(f"   ✅ Edge cases present: empty cells={has_empty_cells}, special chars={has_special_chars}, large numbers={has_large_numbers}")
    else:
    ...
    if error_files:
        print(f"   ✅ Found {len(error_files)} error files: {', '.join([f.name for f in error_files])}")
    else:
        print("   ⚠️  No error files found (this is acceptable for CSV-only version)")
    ...
    criteria_results["multiple_formats"] = {
        ...
        "files": {"csv": len(csv_files), "xls": len(xls_files), "xlsx": len(xlsx_files)}
    }
    ...
    criteria_results["data_dictionary"] = {
        ...
        "meets_criteria": len(dict_files) >= 6,  # Should have dictionaries for all data types
        "files": [f.name for f in dict_files]
    }
    print(f"   ✅ Found {len(dict_files)} data dictionary files: {', '.join([f.name for f in dict_files])}")
    ...
    criteria_results["encodings"] = {
        ...
        "meets_criteria": len(utf8_files) > 0 and len(ascii_files) > 0 and len(utf16_files) > 0,
        "files": {"utf8": len(utf8_files), "ascii": len(ascii_files), "utf16": len(utf16_files)}
    }
    print(f"   ✅ Found UTF-8: {len(utf8_files)}, ASCII: {len(ascii_files)}, UTF-16: {len(utf16_files)} files")
    
    # Test 12: Create script for reproducible data generation
    print("\n12. Testing reproducible data generation script...")
    script_files = [Path("generate_csv_data.py"), Path("generate_test_data.py")]
    ...
    print(f"   ✅ Found {len(existing_scripts)} generation scripts: {', '.join([f.name for f in existing_scripts])}")
    ...
    criteria_results["property_types"] = {
        ...
        "files_per_type": {pt: len(list(test_resources_dir.glob(f"*_{pt}.csv"))) for pt in property_types}
    }
    ...
    if comparables_files:
        ...
        for prop in properties[:5]:  # Check first 5 properties
            try:
                ...
                if total_sqft > 0 and abs(price_per_sqft - (sale_price / total_sqft)) > 1:
        ...
        print(f"   ✅ Financial calculations appear realistic: {realistic_calculations}")
    else:
    ...
    non_dict_files = [f for f in all_files if not f.name.endswith("_fields.csv") and f.is_file()]
    
    criteria_results["file_storage"] = {
        "total_files": len(non_dict_files),
        "directory": str(test_resources_dir),
        "meets_criteria": len(non_dict_files) >= 20,  # Should have many test files
    ...
    for criterion, result in criteria_results.items():
        status = "✅ PASS" if result.get("meets_criteria", False) else "❌ FAIL"
