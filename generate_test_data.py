...
from data_generators import (
    RentRollGenerator,
    ComparablesGenerator,
    OperatingExpensesGenerator,
    TenantRosterGenerator,
    NOIGenerator,
...
class SRSDataGenerator:
    ...
    def generate_all_data(
        self, include_edge_cases: bool = True, include_errors: bool = True
    ):
        ...
        for prop_type in property_types:
            ...
            self._generate_property_type_data(
                prop_type, include_edge_cases, include_errors
            )
    ...
    def _generate_property_type_data(
        self,
        ...
        units = self.rent_roll_gen.generate_rent_roll(
            num_units=random.randint(50, 500), property_type=prop_type
        )
        rent_roll_data = self.rent_roll_gen.rent_roll_to_dict(units)

        # Export in multiple formats and encodings
        self._export_rent_roll(
            rent_roll_data, prop_name, include_edge_cases, include_errors
        )
        ...
        comparables = self.comparables_gen.generate_comparables(
            num_properties=random.randint(20, 50), property_type=prop_type
        )
        comparables_data = self.comparables_gen.comparables_to_dict(
            comparables
        )
        ...
        self._export_operating_expenses(
            expenses_data, prop_name, include_edge_cases, include_errors
        )

        # 4. Tenant Roster
        print(f"  👥 Generating tenant roster for {prop_name}...")
        tenants = self.tenant_gen.generate_tenant_roster(
            num_tenants=random.randint(100, 200), property_type=prop_type
        )
        tenants_data = self.tenant_gen.tenant_roster_to_dict(tenants)

        self._export_tenant_roster(
            tenants_data, prop_name, include_edge_cases, include_errors
        )

        # 5. Historical NOI Data (3-5 years)
        print(f"  📈 Generating NOI data for {prop_name}...")
        years = random.randint(3, 5)
        noi_data = self.noi_gen.generate_noi_data(
            years=years, property_type=prop_type
        )
        noi_data_dict = self.noi_gen.noi_data_to_dict(noi_data)

        self._export_noi_data(
            noi_data_dict, prop_name, include_edge_cases, include_errors
        )

        # 6. Market Analysis
        print(f"  📊 Generating market analysis for {prop_name}...")
        markets = self.market_gen.generate_market_analysis(
            num_markets=random.randint(8, 15), property_type=prop_type
        )
        markets_data = self.market_gen.market_analysis_to_dict(markets)

        self._export_market_analysis(
            markets_data, prop_name, include_edge_cases, include_errors
        )

    def _export_rent_roll(
        self,
        ...
        file_path = self.exporter.export_excel_legacy(
            data, f"rent_roll_{prop_name}_legacy"
        )
        self.generated_files.append(file_path)

        # Error files for validation testing
        if include_errors:
            error_files = self.exporter.create_error_files(
                data, f"rent_roll_{prop_name}_errors"
            )
    ...
    def _export_comparables(self, data: List[Dict[str, Any]], prop_name: str):
        ...
        file_path = self.exporter.export_excel(
            data, f"comparables_{prop_name}"
        )
        self.generated_files.append(file_path)

        file_path = self.exporter.export_excel_legacy(
            data, f"comparables_{prop_name}_legacy"
        )
        self.generated_files.append(file_path)
    ...
    def _export_operating_expenses(
        self,
        data: List[Dict[str, Any]],
        prop_name: str,
        include_edge_cases: bool,
        ...
        # Excel format
        file_path = self.exporter.export_excel(
            data, f"operating_expenses_{prop_name}"
        )
        self.generated_files.append(file_path)
        ...
        if include_errors:
            error_files = self.exporter.create_error_files(
                data, f"operating_expenses_{prop_name}_errors"
            )
            self.generated_files.extend(error_files)

    def _export_tenant_roster(
        self,
        data: List[Dict[str, Any]],
        prop_name: str,
        include_edge_cases: bool,
        ...
        # Excel format
        file_path = self.exporter.export_excel(
            data, f"tenant_roster_{prop_name}"
        )
        self.generated_files.append(file_path)
        ...
        if include_errors:
            error_files = self.exporter.create_error_files(
                data, f"tenant_roster_{prop_name}_errors"
            )
            self.generated_files.extend(error_files)

    def _export_noi_data(
        self,
        data: List[Dict[str, Any]],
        prop_name: str,
        include_edge_cases: bool,
        ...
        if include_errors:
            error_files = self.exporter.create_error_files(
                data, f"noi_data_{prop_name}_errors"
            )
            self.generated_files.extend(error_files)

    def _export_market_analysis(
        self,
        data: List[Dict[str, Any]],
        prop_name: str,
        include_edge_cases: bool,
        ...
        # Excel format
        file_path = self.exporter.export_excel(
            data, f"market_analysis_{prop_name}"
        )
        self.generated_files.append(file_path)
        ...
        if include_errors:
            error_files = self.exporter.create_error_files(
                data, f"market_analysis_{prop_name}_errors"
            )
            self.generated_files.extend(error_files)
    ...
    def _generate_data_dictionaries(self):
        ...
        for dict_name, fields_dict in dictionaries.items():
            file_path = self.exporter.export_data_dictionary(
                fields_dict, dict_name
            )
            self.generated_files.append(file_path)
    ...
    def _generate_summary_report(self):
        ...
        for file_path in self.generated_files:
            ...
            if "rent_roll" in filename:
                summary_data["files_by_type"]["rent_roll"] = (
                    summary_data["files_by_type"].get("rent_roll", 0) + 1
                )
            elif "comparables" in filename:
                summary_data["files_by_type"]["comparables"] = (
                    summary_data["files_by_type"].get("comparables", 0) + 1
                )
            elif "operating_expenses" in filename:
                summary_data["files_by_type"]["operating_expenses"] = (
                    summary_data["files_by_type"].get("operating_expenses", 0)
                    + 1
                )
            elif "tenant_roster" in filename:
                summary_data["files_by_type"]["tenant_roster"] = (
                    summary_data["files_by_type"].get("tenant_roster", 0) + 1
                )
            elif "noi_data" in filename:
                summary_data["files_by_type"]["noi_data"] = (
                    summary_data["files_by_type"].get("noi_data", 0) + 1
                )
            elif "market_analysis" in filename:
                summary_data["files_by_type"]["market_analysis"] = (
                    summary_data["files_by_type"].get("market_analysis", 0) + 1
                )

            # Count by property type
            for prop_type in ["multifamily", "office", "retail"]:
                if prop_type in filename:
                    summary_data["files_by_property_type"][prop_type] = (
                        summary_data["files_by_property_type"].get(
                            prop_type, 0
                        )
                        + 1
                    )
            ...
            for encoding in ["utf-8", "ascii", "utf-16"]:
                if encoding in filename:
                    summary_data["files_by_encoding"][encoding] = (
                        summary_data["files_by_encoding"].get(encoding, 0) + 1
                    )

            # Count by format
            if filename.endswith(".csv"):
                summary_data["files_by_format"]["csv"] = (
                    summary_data["files_by_format"].get("csv", 0) + 1
                )
            elif filename.endswith(".xlsx"):
                summary_data["files_by_format"]["xlsx"] = (
                    summary_data["files_by_format"].get("xlsx", 0) + 1
                )
            elif filename.endswith(".xls"):
                summary_data["files_by_format"]["xls"] = (
                    summary_data["files_by_format"].get("xls", 0) + 1
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate synthetic test data for SRS Document Generator"
    )
    parser.add_argument(
        "--output-dir",
        "-o",
    ...
    parser.add_argument(
        "--no-errors", action="store_true", help="Skip generating error files"
    )
